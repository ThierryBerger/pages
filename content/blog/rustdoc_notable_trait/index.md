...
Trait-heavy framework APIs (Bevy, but also Diesel, axum, embedded HALs) have poor discoverability in default rustdoc. From a type page it is hard to answer: *is this a `Component`? a `Resource`? a `SystemParam`?* Trait impls are listed flatly, far from the type's header, and conceptual grouping of items by trait role does not exist.

Bevy has been experimenting with augmenting rustdoc output to surface these concepts. We would like rustdoc-team input on the approach before investing further.
...
- Shipping JS from a crate to alter rustdoc output is... controversial, and will:tm: be forbidden in the future.
